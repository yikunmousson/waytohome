...
![四条走廊空间走向（去程）](web/corridors-thumb-outbound-v2.png)

> 文件名里的 `v2` 是 `config.yaml` 里的 `route_version`。改了途经点、把它递增之后，
> 记得同步改上面这行链接（以及 `web/` 里留下的旧版本图可以删掉）。
...
产出 `web/corridors-thumb-<方向>-v<路线版本>.png`，旁边还有一份同名 `.json` 记录尺寸、
...
- **为什么把 `route_version` 编进文件名**：改了途经点之后，旧版本的图还躺在 `web/` 里。
  如果文件名不变，前端拿到的就是「旧路线的图配新路线的数据」—— 看着对，其实是错的；
  浏览器缓存还会让旧图更难清掉。编进文件名之后，**文件名对不上就是 404**，
  页面自然走到「这条路线还没生成缩略图」的提示，不会新旧混搭。
  （定时任务里缩略图那步是 `continue-on-error`，失败了不会挡住采集，所以这道保险是必要的。）
...
注意新图会以新的版本号命名（如 `-v3.png`），旧的那张会被自动清掉；
看板按 `data.json` 里的 `route_version` 取图，所以两者必须同时对。
